...
P2 probabilities per unique patient across seeds before calculating final
...
def validate(artifact_root: Path, freeze_path: Path, output_dir: Path) -> dict:
    ...
    expected_seeds = set(
        range(
            int(freeze["formal_seeds"]["start"]),
            int(freeze["formal_seeds"]["stop_inclusive"]) + 1,
        )
    )
    ...
    for summary_path in summary_paths:
        ...
        if not (
            oof["candidate"].eq("P2").all()
            and oof["profile"].eq("rhythm_safe").all()
            and oof["model"].eq("elasticnet").all()
        ):
        ...
        raw_metric = _metrics(oof["y_true"].to_numpy(), oof["probability"].to_numpy())
        ...
        if int(included.sum()) != 878:
            raise AssertionError(f"seed {seed}: manifest included count is not 878")
        included_ids = set(manifest.loc[included, "patient_id"].astype(str))
        ...
        if int((included & evaluable_positive).sum()) != 37:
            raise AssertionError(f"seed {seed}: included positive count drift")
        af_manifest = manifest["af_flag"].astype(bool)
        if int((included & af_manifest).sum()) != 164:
            raise AssertionError(f"seed {seed}: included AF count drift")
        if int((included & af_manifest & evaluable_positive).sum()) != 10:
            raise AssertionError(f"seed {seed}: included AF-positive count drift")
        excluded_positive = manifest.loc[~included & evaluable_positive]
        ...
        manifest_check = manifest[
            ["patient_id", "endpoint_state", "label", "af_flag", "included", "exclusion_reason"]
        ].copy()
        ...
        all_oof.append(
            oof[["patient_id", "y_true", "probability", "af_flag", "pvc_count_24h", "seed"]].copy()
        )
        checked_paths.extend([summary_path, oof_path, manifest_path])

    if observed_seeds != expected_seeds:
        raise AssertionError(
            f"seed set mismatch: missing={sorted(expected_seeds-observed_seeds)}, "
    ...
    result = {
        ...
        "bootstrap_note": (
            "95% percentile intervals resample the 878 unique patients after averaging each patient's "
            "100 OOF probabilities; they are distinct from the 100-seed split-sensitivity percentiles."
        ),
    ...
    (output_dir / "defensive_validation.json").write_text(
        json.dumps(result, ensure_ascii=False, indent=2, allow_nan=False), encoding="utf-8"
    )

    markdown = "# P2 frozen candidate defensive validation\n\n"
    ...
    markdown += (
        f"- OOF integrity: {len(repeated):,} rows = {len(unique_patient)} unique patients × 100 seeds; "
        "one OOF prediction per patient per seed.\n"
    )
    markdown += (
        f"- Frozen population: {len(unique_patient)} patients, {int(unique_patient['y_true'].sum())} positives; "
        f"AF {int(unique_patient['af_flag'].sum())} patients / "
        f"{int((unique_patient['af_flag'].astype(bool) & unique_patient['y_true'].eq(1)).sum())} positives.\n\n"
    )
    markdown += "## Unique-patient aggregation across 100 seeds\n\n"
    markdown += (
        "The 100 OOF probabilities for each patient were averaged first; metrics below therefore use each "
        "patient exactly once.\n\n"
    )
    markdown += "| subgroup | N | positives | AUC | AP | Brier | Brier Skill | calibration slope | calibration intercept |\n"
    markdown += "|---|---:|---:|---:|---:|---:|---:|---:|---:|\n"
    for name in ("overall", "AF", "sinus"):
        metric = unique_json[name]
        markdown += (
            f"| {name} | {metric['patient_count']} | {metric['positive_count']} | {metric['AUC']:.4f} | "
            f"{metric['AP']:.4f} | {metric['Brier']:.4f} | {metric['BrierSkill']:.4f} | "
            f"{metric['calibration_slope']:.4f} | {metric['calibration_intercept']:.4f} |\n"
        )
    markdown += (
        "\nThe AF subgroup remains exploratory because it contains only 10 unique positive patients. "
        "Patient-bootstrap intervals are stored in `defensive_validation.json`; the 100-seed split "
        "distributions are stored separately and must not be called population confidence intervals.\n"
    )
    markdown += (
